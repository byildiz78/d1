...
export function FormDistributionChart({ data }: FormDistributionChartProps) {
  ...
  // Verileri yüzdeye göre sırala ve %7'den küçükleri "Diğer" altında topla
  const sortedData = [...data].sort((a, b) => b.count - a.count);
  const processedData = sortedData.reduce((acc, item) => {
    const percentage = (item.count / total) * 100;
    if (percentage < 7) {
      const otherIndex = acc.findIndex(item => item.form === 'Diğer');
      if (otherIndex === -1) {
        acc.push({ form: 'Diğer', count: item.count });
      } else {
